...
fn build_ui(app: &Application) {
    ...
    let albums_grid_ref = albums_grid.clone();
    let track_list_container_ref = track_list_container.clone();
    let main_column_container_ref = main_column_container.clone();
    let page_menu_container_ref = page_menu_container.clone();
    page_button_albums.connect_clicked(move |_| {
        switch_page(
            "albums",
            &albums_grid_ref.clone(),
            &track_list_container_ref.clone(),
            &main_column_container_ref,
            &page_menu_container_ref,
        );
    });
...
fn collect_album_lib(
    ...
    for album in lib {
        println!("instancing: {}", album.title);
...
fn instance_track_list(
    ...
    while let Some(child) = main_window.last_child() {
        main_window.remove(&child);
    }

    main_window.append(page_menu_container);
